...
visfilesdir = '/scratch/yiweix3/eDisk/OphIRS43/UVDATA/'
imagefile = '/scratch/yiweix3/eDisk/OphIRS43/OphIRS43_SBLB_continuum_robust_0.5.image.tt0.fits'
...
def ttauri_ring_model(params, xp, yp):
    yi = yp / numpy.cos(params["incl"])
    rp = numpy.sqrt(xp**2 + yi**2)
    rp2 = rp - params["r_w"]
    phi = numpy.arctan2(yi,xp)
    
    return params["flux"] * numpy.exp(-(rp2**2 / (2 * params["r_w2"]**2))) * \
    	    (1.0 + params["a"] * numpy.cos(2.0 * phi)**2)
...
names = {\
	...
	"exponentially_tapered_powerlaw2_ring2":["x0","y0",\
        ...
        "exponentially_tapered_broken_powerlaw":["x0","y0",\
                "logr_w","incl","pa","logflux","gamma_in",\
                "gamma_out", "logdelta", "logr_exp", "gamma2"],\
        ...
        "symmetric_divgauss_gapped_flared_asymmetric_broken_powerlaw_rectangle_vartrunc_varsmooth":["x0",\
                ...
                "gamma_out2","logx_t","logdelta","gammax","gammay","scale",\
                "logsmooth","xgap","logwgap","logdelta_gap"],\
        "ttauri_ring":["x0","y0","logr_w","incl","pa","logflux","logr_w2","a"],\
        }
...
################################################################################
#
# Revision: define a combine_trinary_names function, accept three input model_name_1, model_name_2, model_name_3
#	    for params in model_name_2 and _3, add suffix _2 or _3
#	    Combine the params of three models into a list, return the list
#
################################################################################
def combine_trinary_names(model_name_1, model_name_2, model_name_3):
    names_1 = names[model_name_1]
    names_2 = [f"{param}_2" for param in names[model_name_2]]
    names_3 = [f"{param}_3" for param in names[model_name_3]]
...
priors_2 = {
	...
	"logr_r_2":[-3.0,1.0],\
        ...
        "logx_we_2":["logr_w_2",3.0],\
        "logflux_envelope_2":[-5.,"logflux_2"],\
        }
...
priors_3 = {
	"x0_3": [-1.7, -1.0],
    	...
    	"b_3": [-2.0, 0.0],
    	"c_3": [0.0, 3.0],
    	"incl_3": [0.0, numpy.pi/2],
    	"pa_3": [0.0, numpy.pi],
    	"phi_c_3": [-numpy.pi, numpy.pi],
    	"phi_w_3": [0.0, numpy.pi],
    	"phi_o_3": [-numpy.pi, numpy.pi],
    	"logflux_3": [-4.0,0.0],
    	"logf_a_3": [-6.0, -0.3],
    	"gamma_3": [0.0, 2.0],
    	"gamma1_3": [-2.0, 2.0],
    	...
    	"gamma_in_3": [-2.0, 2.0],
    	"gamma_out_3": [-2.0, 2.0],
    	"gamma_out1_3": [-1.0, "gamma_in_3"],
    	"gamma_out2_3": [-1.0, "gamma_in_3"],
    	"logx_t_3": [-2.3, "logx_w_3"],
    	"logdelta_3": [-2.0, 2.0],
    	"xgap_3": ["logx_t_3", "logx_w_3"],
    	"logwgap_3": [-3.0, -1.0],
    	"logdelta_gap_3": [-3.0, 0.0],
    	"logx_we_3": ["logr_w_3",3.0],
    	"logflux_envelope_3": [-5.,"logflux_3"],
}

priors = {**priors_1, **priors_2, **priors_3}
...
################################################################################
#
# Revision: Define a trinary model.
#
...
#	    so that p contains the values of all parameters of three disks, ordered
#	    
#           trinary_model() would slice p into three parts, with parameters of each disk in each part
#	    call model() three times for three disks, combine the visibilities of three disks and return
#
################################################################################

def trinary_model(u, v, p, npix, pixelsize, output, freq, model_name_1, model_name_2, model_name_3):
    names_1 = names[model_name_1]
    names_2 = [f"{param}_2" for param in names[model_name_2]]
    names_3 = [f"{param}_3" for param in names[model_name_3]]
    
    params_value_1 = p[:len(names_1)]
    params_value_2 = p[len(names_1):len(names_1) + len(names_2)]
    params_value_3 = p[len(names_1) + len(names_2):]
    ...
    vis_3 = model(u, v, params_value_3, npix, pixelsize, output, freq, model_name_3)
    
    if output == "data":
        trinary_vis = uv.Visibilities(
            vis_1.u, vis_1.v, vis_1.freq,
            vis_1.real + vis_2.real + vis_3.real,
            vis_1.imag + vis_2.imag + vis_3.imag,
            vis_1.weights
            )
    else:
        trinary_vis = vis_1 + vis_2 + vis_3

    return trinary_vis

################################################################################
#
# Revision: Define a trinary likelihood model.
#	    Now accept model_name_1, model_name_2, model_name_3 and call trinary_model()
#
################################################################################

def lnlike(p, x, y, z, zerr, npix, pixelsize, output, freq, model_name_1, model_name_2, model_name_3):
    m = trinary_model(x, y, p, npix=npix, pixelsize=pixelsize, output=output, \
            freq=freq, model_name_1=model_name_1, model_name_2=model_name_2, model_name_3=model_name_3)
...
################################################################################
#
# Revision: if param in ("x0", "x0_2", "x0_3"):
#
# Comments: For x0, y0, they are defined as 0.0 right now, assume this is true for all disks
#
#	    For params, it is input in dynesty.NestedSampler by:
#		ptform_args=(source, x0, y0, names[model_name])
#
#	    I'll call function combine_trinary_names(model_name_1, model_name_2, model_name_3) to replace names[model_name]
#	    So that ptform() accept parameters of three models, where the second model params has suffix _2, the third model params has suffix_3
#	    
#           ptform would now return an numpy array p
#	    that contains the values of all parameters of three sources, ordered
#
################################################################################
...
def ptform(u, source, x0, y0, params):
    ...
    for i, param in enumerate(params):
        ...
        if param in ("x0", "x0_2", "x0_3"):
            p[param] += x0
        elif param in ("y0", "y0_2", "y0_3"):
            p[param] += y0
...
#           directly let model_name_1, model_name_2, model_name_3 = args.model
#
################################################################################
...
def parse_model_names(model_input):
    ...
    if len(model_names) == 3:
        return model_names[0], model_names[1], model_names[2]
    else:
        raise argparse.ArgumentTypeError("Invalid model input format. Expected 'model1&model2&model3'.")
...
model_name_1, model_name_2, model_name_3 = args.model
...
#
# Revision: I already set model_name_1, model_name_2, model_name_3 = args.model
#	    No longer need intertaion for model_name
#
#	    Call combine_trinary_names(), store all parameters names into trinary_names
#	    
#	    In for loop, replace all names[model_name] with trinary_names
#
################################################################################

# Call combine_trinary_names(), store all parameters names into trinary_names
trinary_names = combine_trinary_names(model_name_1, model_name_2, model_name_3)
...
#    vis = uv.average(data, gridsize=4096, binsize=4000, mfs=True)
    ...
    print(trinary_names)
    ndim, nwalkers = len(trinary_names), 400
...
#    else:
    ...
    xlabels = [labels[p] for p in trinary_names]
    ...
    if args.resume:
        ...
        sampler = dynesty.NestedSampler(lnlike, ptform, ndim, nlive=500, \
            ...
            model_name_1, model_name_2, model_name_3), ptform_args=(source, x0, y0, trinary_names), \
    ...
    for i, name in enumerate(trinary_names):
    ...
    ############################################################################
    #
    # Revision: Now params contains all trinary disks parameter best fits
    #           We can call trinary_model() to get the overall visibility
    #
    ############################################################################
    ...
    params_native[len(names[model_name_1])] = 0.0
    params_native[len(names[model_name_1])+1] = 0.0
    params_native[len(names[model_name_1])+4] = numpy.pi/2
    params_native[len(names[model_name_1])] = 0.0
    params_native[len(names[model_name_1])+len(names[model_name_2])+1] = 0.0
    params_native[len(names[model_name_1])+len(names[model_name_2])+4] = numpy.pi/2
   
    # Print native model image for the main source.
    model_main_image_native = model(x, y, params_native[:len(names[model_name_1])], \
    	output="image", freq=vis.freq[0], npix=256*4, pixelsize=dx/2, model_name=model_name_1)
    model_main_image_native.header = image.header
    model_main_image_native.asFITS().writeto("model_main_native.fits",overwrite=True)

    # Print native model image for the secondary source.
    model_sec_image_native = model(x, y, params_native[len(names[model_name_1]):len(names[model_name_1])+len(names[model_name_2])], \
    	output="image", freq=vis.freq[0], npix=256*4, pixelsize=dx/2, model_name=model_name_2)
    model_sec_image_native.header = image.header
    model_sec_image_native.asFITS().writeto("model_sec_native.fits",overwrite=True)
    
    # Print native model image for the tertiary source.
    model_ter_image_native = model(x, y, params_native[len(names[model_name_1])+len(names[model_name_2]):], \
    	output="image", freq=vis.freq[0], npix=256*4, pixelsize=dx/2, model_name=model_name_3)
    model_ter_image_native.header = image.header
    model_ter_image_native.asFITS().writeto("model_ter_native.fits",overwrite=True)
    
    model_vis = trinary_model(x, y, params, output="data", freq=vis.freq[0], npix=256*4, pixelsize=dx/2, \
            model_name_1=model_name_1, model_name_2=model_name_2, model_name_3=model_name_3)
...
# cleaning down to some value in the model image
    model_image = uv.clean(model_vis, imsize=N, pixel_size=dx, \
            ...
            weighting="robust", robust=0.5, maxiter=50, \
