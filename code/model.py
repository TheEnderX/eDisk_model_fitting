...
visfilesdir = '/scratch/yiweix3/eDisk/OphIRS63/UVDATA/'
imagefile = '/scratch/yiweix3/eDisk/OphIRS63/OphIRS63_SBLB_continuum_robust_0.5.image.tt0.fits'
...
def broken_powerlaw_model(params, xp, yp):
    ...
    return params["flux"] * (((numpy.sqrt(rp**2 + params["smooth"]**2)/params["r_w"])**(-params["gamma_in"])) * \
    	    (0.5 * (1. + ((numpy.sqrt(rp**2 + params["smooth"]**2) / params["r_w"])**(1./params["delta"])))) ** \
    	    (-(params["gamma_out"]-params["gamma_in"])*params["delta"]))
...
def exponentially_tapered_broken_powerlaw_ring2_model(params, xp, yp):
    yi = yp / numpy.cos(params["incl"])
    rp = numpy.sqrt(xp**2 + yi**2)
    rp2 = rp - params["r_c"]
    rp3 = rp - params["r_c3"]
...
model_functions = {\
	...
	"broken_powerlaw":broken_powerlaw_model, \
        ...
        "exponentially_tapered_broken_powerlaw_ring2":exponentially_tapered_broken_powerlaw_ring2_model, \
...
names = {\
	...
	"exponentially_tapered_powerlaw2_ring2_gaussian":["x0","y0",\
        ...
        "exponentially_tapered_broken_powerlaw_ring2":["x0","y0",\
                "logr_w","incl","pa","logflux","gamma_in",\
                "gamma_out", "delta", "logr_exp", "gamma2",\
                "logr_c","logr_w2","a",\
                "logr_c3","logr_w3","b"],\
...
#    else:
    ...
    if args.resume:
        ...
        sampler = dynesty.NestedSampler(lnlike, ptform, ndim, nlive=500, \
            logl_args=(x, y, z, zerr, 256*8, dx/2, "concat", vis.freq[0], \
    ...
    # Print native model image.
    model_image_native = model(x, y, params_native, output="image", npix=256*8, pixelsize=dx, \
            model_name=model_name)
    model_image_native.header = image.header
    model_image_native.asFITS().writeto("model_native.fits",overwrite=True)


    model_vis = model(x, y, params, output="data", npix=256*8, pixelsize=dx/2, \
            model_name=model_name)
...
# cleaning down to some value in the model image
    model_image = uv.clean(model_vis, imsize=N, pixel_size=dx, \
            ...
            weighting="robust", robust=0.5, maxiter=500, \
    ...
    model_image.asFITS().writeto("model.fits",overwrite=True)
    ...
    residual_image.asFITS().writeto("residuals.fits",overwrite=True)
